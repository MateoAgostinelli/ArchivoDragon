...
| 2026-08-19 | Selector de tema blanco/negro/rojo agregado; "blanco" pasa a ser blanco real | Se implementó un picker de 3 colores en el header para que el visitante elija el fondo directamente (persistido en localStorage). El usuario pidió luego que el tema "blanco" fuera blanco de verdad, no el crema/papel original: `--paper` pasó de `#f4ede2` a `#ffffff` (paper-raised `#f2f0ec`, rule `#ddd8ce`). El tema "negro" (fondo casi negro, rojo subido a `#ff4d3d`) y el tema "rojo" (fondo `#8f1912`, acento rojo invertido a negro para contraste) quedan sin cambios. |
